...
# Keywords that strongly indicate failed testcase
FAILED_TC_KEYWORDS = [
...
# Keywords that strongly indicate bug
BUG_KEYWORDS = [
...
# Error patterns
ASSERTION_PATTERNS = [
    r'assert\s+\d+\.\d+\s*==\s*\d+\.\d+',  # float comparison
    r'assert\s+\d+\s*==\s*\d+',              # int comparison
    r"assert\s+'[^']+'\s+in\s+'[^']+'",      # string in check
    r'assert\s+\w+\s*==\s*\w+',             # variable comparison
]

def extract_features(logs: str, description: str, code: str) -> np.ndarray:
    """Extract numerical features from test failure data"""
    ...
    # ── Feature group 1: Exception type indicators ──
    ...
    features.append(1 if 'nullpointerexception' in logs_lower or 'nullreferenceexception' in logs_lower else 0)
    features.append(1 if 'integrityerror' in logs_lower else 0)
    features.append(1 if 'connectionrefusederror' in logs_lower else 0)
    ...
    # ── Feature group 2: Test framework detection ──
    ...
    features.append(1 if 'db.' in code_lower or 'query(' in code_lower or 'sqlalchemy' in combined else 0)
    features.append(1 if 'by.id' in code_lower or 'by.xpath' in code_lower or 'by.css' in code_lower else 0)
    features.append(1 if 'webdriverwait' in code_lower or 'expected_conditions' in code_lower else 0)
    features.append(1 if 'pytest' in combined or 'def test_' in code_lower else 0)

    # ── Feature group 3: Bug indicators ──
    ...
    features.append(1 if '401' in logs_lower and 'assert' in code_lower and '200' in code_lower else 0)

    # ── Feature group 4: Failed testcase indicators ──
    features.append(1 if any(kw in logs_lower for kw in FAILED_TC_KEYWORDS) else 0)
    features.append(1 if 'invalid' in logs_lower and ('selector' in logs_lower or 'xpath' in logs_lower) else 0)
    features.append(1 if 'locator' in desc_lower or 'selector' in desc_lower else 0)
    features.append(1 if 'no such table' in logs_lower else 0)
    features.append(1 if 'chrome not reachable' in logs_lower or 'session not created' in logs_lower else 0)

    # ── Feature group 5: Code pattern analysis ──
    features.append(1 if 'find_element' in code_lower else 0)
    features.append(1 if 'assert' in code_lower else 0)
    features.append(len(re.findall(r'assert', code_lower)))  # count of assertions
    features.append(1 if 'time.sleep' in code_lower else 0)
    features.append(1 if 'hardcoded' in desc_lower or 'hardcode' in desc_lower else 0)
    features.append(len(code.split('\n')))  # code length in lines

    # ── Feature group 6: Description semantic hints ──
    features.append(1 if 'verif' in desc_lower and ('redirect' in desc_lower or 'login' in desc_lower) else 0)
    features.append(1 if 'verif' in desc_lower and 'total' in desc_lower else 0)
    features.append(1 if 'clicks' in desc_lower or 'click' in desc_lower else 0)

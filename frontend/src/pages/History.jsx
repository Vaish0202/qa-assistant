...
export default function History() {
  ...
  const getStatusBadge = (classification) => {
    ...
    if (classification === 'failed_testcase') return (
    ...
    <div className="p-8">
      ...
      <div className="flex gap-3 mb-6">
        <div className="flex-1 relative">
          <svg className="w-4 h-4 absolute left-3 top-3 text-[#6b7280]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-[#2a2a3d]">
              {['ID', 'Test Case', 'Status', 'Classification', 'Severity', 'Analyzed On', 'Actions'].map(h => (
                <th key={h} className="text-left text-xs font-medium text-[#6b7280] px-4 py-3">
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {loading && (
              <tr>
                <td colSpan={7} className="text-center text-[#6b7280] py-8 text-sm">
                  Loading...
                </td>
              </tr>
            )}
            ...
            {filtered.map((item, i) => (
              <>
                <tr
                  key={item.id}
                  onClick={() => item.session_id ? navigate(`/chat/${item.session_id}`) : setExpanded(expanded === item.id ? null : item.id)}
                ...
                >
                  ...
                  <td className="px-4 py-3 text-sm text-[#9ca3af]">
                    {item.classification === 'bug' ? 'Product Bug' :
                     item.classification === 'failed_testcase' ? 'Test Issue' : '—'}
                  </td>
                  <td className="px-4 py-3">
                    ...
                    <td colSpan={7} className="px-6 py-4">
                      <div className="text-sm space-y-1">
                        <p className="text-[#6b7280]">
                          Full ID: <span className="text-white font-mono">{item.id}</span>
                        </p>
                        <p className="text-[#6b7280]">
                          Framework: <span className="text-indigo-400">{item.framework || 'unknown'}</span>
                        </p>
                        {item.alert && (
        ...
        {filtered.length > 0 && (
          <div className="px-4 py-3 border-t border-[#2a2a3d] flex justify-between items-center">
            ...
            <div className="flex gap-1">
              <button className="w-7 h-7 bg-indigo-600 text-white text-xs rounded flex items-center justify-center">1</button>
            </div>
          </div>
        )}
      </div>

...
export default function ResultCard({ result }) {
  ...
  const confidence = Math.round(
    (result.classification_confidence || 0.96) * 100,
  );

  const [showJiraModal, setShowJiraModal] = useState(false);
  const [jiraResult, setJiraResult] = useState(
    payload.jira_ticket
      ? { ticket_key: payload.jira_ticket, ticket_url: payload.jira_url }
      : null,
  );

  
  const jiraPayloadReady = payload.jira_payload_ready || payload;
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-[#0f0f1a] rounded-xl p-3 text-center">
              ...
              <p
                className={`text-sm font-bold ${isBug ? "text-red-400" : "text-yellow-400"}`}
              >
              ...
              <>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div className="space-y-2">
                      ...
                      {(payload.steps_to_reproduce || []).map((s, i) => (
                        <p key={i} className="text-[#9ca3af] text-xs">
                          • {s}
                        </p>
                      ))}
                    </div>
                ...
                <div className="bg-[#13131f] border border-[#2a2a3d] rounded-2xl p-5">
                  ...
                  <p className="text-[#9ca3af] text-sm">
                    {payload.suggested_fix || "Review the application logic."}
                  </p>
                </div>
                ...
                <div className="bg-[#13131f] border border-[#2a2a3d] rounded-2xl p-5">
                  ...
                  <div className="text-[#9ca3af] text-sm">
                    {activeTab === "overview" && (
                      <p>
                        {payload.description ||
                          "The test failure indicates an application-level bug that needs developer attention."}
                      </p>
                    )}
                    {activeTab === "logs" && (
                    ...
                    {activeTab === "response" && (
                      <p className="text-[#9ca3af] text-sm">
                        Bug type: {payload.bug_type} | Framework:{" "}
                        {result.framework}
                      </p>
                    )}
                  </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-[#2a2a3d]">
                      ...
                      <div className="flex gap-2 flex-wrap">
                        {[
                          ...
                          <button
                            key={s}
                            className="text-xs bg-[#1e1e30] border border-[#2a2a3d] text-[#9ca3af] px-3 py-1.5 rounded-lg hover:text-white transition"
                          >
                            {s}
                          </button>
                        ))}
